...
shinyServer(function(input, output) {
  ...
  getNav <- reactive({
	...
	for (j in 1:n.obs) {
		nav[j + 1, ] = nav[j, ] * (1 + monthly.invest.returns[j, ] - monthly.inflation.returns[j, ]) + monthly.deposits
	}
  ...
  output$distPlot <- renderPlot({
	...
	matplot(nav, type = 'l', las = 1, xlab = 'Months', ylab = 'Millions',
		main = 'Projected Value of Portfolio over Time')
